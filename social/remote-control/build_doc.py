...
POST = """אמ!לק -> את /remote-control כולם מכירים. מה שפחות מכירים: יש שורה אחת שהופכת אותו לתמיד דלוק. אוטומטית, בכל סשן, בלי להקליד כלום שוב.

כבר שבועות שאני עושה אותו טקס: פותח טרמינל, מריץ קלוד קוד, מקליד /remote-control. וכל כמה ימים שוכח - יוצא מהבית, והסשן תקוע על אישור הרשאה כשאני כבר לא ליד המחשב. הפעם היחידה שתשכח היא בדיוק הפעם שזה חשוב.

היום נשבר לי. הלכתי לבדוק אם אפשר להדליק את זה כברירת מחדל. אפשר. שורה אחת, ב-~/.claude/settings.json:

"remoteControlAtStartup": true

זהו. עכשיו /remote-control תמיד דלוק. כל סשן עולה כבר מחובר, בלי שאני נוגע בכלום.

והדבר הלא-מובן-מאליו: כשזה תמיד דלוק, זה משנה מתי אני מתחיל משימות. פתאום אני מריץ ריפקטור כבד דווקא לפני שאני יוצא מהבית, ומאשר לו צעדים מהטלפון בדרך. המחשב הוא ה-runner, הנייד הוא השלט - כברירת מחדל, לא כמשהו שאני זוכר להדליק.
